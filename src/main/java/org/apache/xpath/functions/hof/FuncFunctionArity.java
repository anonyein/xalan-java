...
public class FuncFunctionArity extends FunctionDef1Arg
{
				...
				if (funcId != null) {					
					...
					if (funcDefinedArity.contains(arity)) {
						...
						throw new TransformerException("XPST0017 : An XSL function definition for function reference {" 
					                                                                                     + fNamespace + "}" + localName + "#" + arity + " not found.", srcLocator);	
					}
				}
			}
			else if (xObj instanceof XPathInlineFunction) {
	...
	/**
	 * Method definition, to get an XPath function's arity for named 
	 * function reference.
	 * 
	...
	private XObject getFunctionArityXPathNamedFuncReference(XPathNamedFunctionReference xpathNamedFunctionReference, 
		...
		if ((XPathStaticContext.XPATH_BUILT_IN_FUNCS_NS_URI).equals(funcNamespace) || (XPathStaticContext.XPATH_BUILT_IN_MATH_FUNCS_NS_URI).equals(funcNamespace) ||
			...
			if ((function == null) && (XMLConstants.W3C_XML_SCHEMA_NS_URI).equals(funcNamespace)) {
				...
				Short[] funcDefinedArityArr = function.getArity();						
